...
const FormularioSeccaoDois = () => {
  ...
  const [encomenda,setEncomenda] = useState<Iencomenda>({tipoEmbalagem :"",operador:"",referencia:"",quantidadePaletes:0,cliente:"",data:"",produtosPalete:"",lote:0})
          ...
          <Form.Item label="Referência" name={["Encomenda","referencia"]} rules={[{required:true,message:"Indique qual a referência da receita"}]}>
            <Select>
              {referênciasFormulário.map((item)=>{
                return(<Select.Option key={item.id} value={item.referencia}>{item.label}</Select.Option>)
              })}
            </Select>
          </Form.Item>
